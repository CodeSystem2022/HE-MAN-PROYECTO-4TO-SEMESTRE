...
function agregarAlCarrito(producto){
  ...
  let memoria = JSON.parse(localStorage.getItem("cervezas"));
  let cantidadProductoFinal;
  //Si no hay localstorage lo creo
  if(!memoria || memoria.length === 0) {
    const nuevoProducto = getNuevoProductoParaMemoria(producto)
    localStorage.setItem("cervezas",JSON.stringify([nuevoProducto]));
    actualizarNumeroCarrito();
    cantidadProductoFinal = 1;
  }
  else {
    //Si hay localstorage me fijo si el artículo ya está ahí
    const indiceProducto = memoria.findIndex(cerveza => cerveza.id === producto.id)
    ...
    localStorage.setItem("cervezas",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("cervezas"));
  if(memoria && memoria.length > 0){
...
function reiniciarCarrito(){
  localStorage.removeItem("cervezas");
  actualizarNumeroCarrito();
}
